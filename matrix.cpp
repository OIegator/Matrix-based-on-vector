...
Matrix Matrix::reverse()
{
    ...
    if(deter) {
        ...
        throw MatrixException("The matrix is degenerate");
    }
    return reversed;
}
...
Matrix Matrix::operator+(const Matrix &m)
{
    if(rows != m.rows && cols != m.cols)
        throw MatrixException("Invalid dimension of matrices");
...
{
    if(cols > 1) {
        if(v.getSize() != this->cols)
                throw MatrixException("Mismatch between matrix and vector sizes");
